...
class App(oreoOS.App):
    ...
    ROWS = (ROW_MASTER, ROW_TAP, ROW_DOUBLE, ROW_FLIP, ROW_FLIP_ACTION, ROW_SHAKE)
    ...
    def draw(self, d):
        ...
        defs = (
            (
                ...
                theme.PRIMARY if master_on else theme.MUTED,
                False,
            ),
            (
                self.ROW_TAP,
                ...
                theme.TEXT_BRIGHT if master_on else theme.MUTED2,
                True,
            ),
            (
                self.ROW_DOUBLE,
            ...
            (
                self.ROW_FLIP_ACTION,
                "  Action",
                _FLIP_LABELS.get(self._flip_action(), self._flip_action()),
                theme.TEAL if master_on else theme.MUTED2,
                True,
            ),
            (
                self.ROW_SHAKE,
        ...
        for idx, label, value, value_color, indent in defs:
            ...
            label_color = (
                theme.TEXT_BRIGHT if (idx == self.ROW_MASTER or master_on) else theme.MUTED2
            )
        ...
        if not master_on:
            ...
            d.text(msg, (SW - len(msg) * 8) // 2, SH - 32, theme.MUTED, scale=1)
