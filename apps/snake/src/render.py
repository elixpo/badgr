...
def load_bg():
    ...
    try:
        m = __import__("apps.snake.assets.optimized.arena", None, None, ["DATA", "W", "H"])
...
def load_food():
    ...
    try:
        m = __import__("apps.snake.assets.optimized.food", None, None, ["DATA", "W", "H"])
        return (bytearray(m.DATA), m.W, m.H)
    except (ImportError, AttributeError):
        return None
...
def draw_arena(d, snake, food, food_sprite):
    ...
    if bg:
        ...
        d.rect(g.ARENA_X, g.ARENA_Y, g.ARENA_W, g.ARENA_H, theme.CARD, fill=True)
    ...
    if food_sprite:
        ...
        d.blit(fdata, fx + (g.CELL - fw) // 2, fy + (g.CELL - fh) // 2, fw, fh)
    else:
        d.rect(fx + 1, fy + 1, g.CELL - 2, g.CELL - 2, theme.PRIMARY, fill=True)
    ...
    for i, (c, r) in enumerate(snake):
        ...
        d.rect(
            g.ARENA_X + c * g.CELL + 1,
            g.ARENA_Y + r * g.CELL + 1,
            g.CELL - 2,
            g.CELL - 2,
            color,
            fill=True,
...
def draw_gameover(d, score, hi, new_hi, blink):
    ...
    title = "GAME OVER"
    d.text(title, (SW - len(title) * 24) // 2, band_y + 14, api.WHITE, scale=3)
    ...
    d.text(score_s, (SW - len(score_s) * 16) // 2, band_y + 52, api.WHITE, scale=2)
    d.text(note, (SW - len(note) * 16) // 2, band_y + 76, note_c, scale=2)

    if int(blink * 2) % 2 == 0:
        msg = "Press A to retry"
        d.text(msg, (SW - len(msg) * 8) // 2, band_y + band_h - 18, api.WHITE)
