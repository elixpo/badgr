...
class App(oreoOS.App):
    ...
    def _on_btn_help(self, btn):
        ...
        if btn == api.BTN_UP:
            ...
            self._help_scroll = min(max(0, len(_HELP) - 1), self._help_scroll + 1)
        else:
            return
        self._dirty = True
    ...
    def _draw_picker(self, d):
        ...
        for i, (name, _) in enumerate(self._files):
            ...
            sel = i == self._sel
            if sel:
                d.rect(6, y, SW - 12, row_h - 2, theme.DOCK_SEL, fill=True)
        ...
        if not self._files:
            ...
            d.text(msg, (SW - len(msg) * 8) // 2, SH // 2 - 4, theme.MUTED, scale=1)
            d.text(
                "press A on '+ How to add'", (SW - 24 * 8) // 2, SH // 2 + 10, theme.MUTED2, scale=1
            )
    ...
    def _draw_view(self, d):
        ...
        for kind, payload, h in self._blocks:
            ...
            if kind == "blank":
                ...
                d.rect(PAD_X, top + 5, SW - 2 * PAD_X, 1, theme.MUTED, fill=True)
                ...
                d.rect(PAD_X - 2, top, SW - 2 * PAD_X + 4, h, theme.CARD, fill=True)
                ...
                _draw_spans(d, spans_, bx + 10, top + 1, scale=1, code_bg=True)
            else:  # para
                _draw_spans(d, payload, PAD_X, top + 1, scale=1, code_bg=True)

            y_cursor += h

        # Right-edge scroll thumb when content overflows.
        widgets.draw_scrollbar(
            d, SW - 3, play_top, 2, play_h, self._total_h, self._scroll, visible=play_h
        )
