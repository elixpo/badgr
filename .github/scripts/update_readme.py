...
MAJOR_KEYWORDS = re.compile(r"\b(breaking|major|redesign|migration)\b", re.IGNORECASE)
...
def is_major_change(title: str, body: str, changed_files: list[str]) -> tuple[bool, list[str]]:
    ...
    auth_files = [
        f for f in changed_files if any(f.startswith(prefix) for prefix in CORE_AUTH_PREFIXES)
    ]
    if auth_files:
