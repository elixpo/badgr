...
def _argparse():
    ...
    p.add_argument("--size", type=int, required=True, help="rendering pixel height (glyph_h)")
    p.add_argument(
        "--max-width", type=int, default=None, help="cap glyph width (default = size * 0.85)"
    )
    p.add_argument("--kern", type=int, default=1, help="inter-glyph spacing in pixels (default 1)")
    p.add_argument(
        "--threshold", type=int, default=80, help="alpha threshold for considering a pixel 'set'"
    )
    return p.parse_args()
...
def main():
    ...
    print(
        "Rasterising %s @ %d px → %dx%d cells (bpr=%d)"
